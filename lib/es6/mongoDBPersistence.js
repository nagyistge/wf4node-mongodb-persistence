...
let common = require("workflow-4-node").common;
...
MongoDBPersistence.prototype.getRunningInstanceHeadersForOtherVersion = async(function*(workflowName, version) {
    yield this._connectAndInit();

    let list = yield this._stateCollection.find({
        "_id.workflowName": {
            $eq: workflowName
        },
        workflowVersion: {
